...
class miescatter:
    ...
    f : str, optional
        Frequency distribution function as a string (e.g., 'x**-2').
    """
    def __init__(self, **kwargs):
        ...
        if type(self.reff) == list and self.f is not None:
            if len(self.reff) != 2:
                print("Invalid input\nPlease enter a valid reff (e.g. reff=[0.01:0.1] or reff=[0.1,0.01])")
                print("Exiting...")
                exit()
    ...
    def miecalc(self):
        ...
        for a in a_:
            if len(a_) != 1:
                frequency = float(self.f.subs({self.x: a}))
            else:
                frequency = 1.0
        ...
        def mie_theta(ITH, a_range):
            ...
            for res in mie_results:
                ...
                if len(a_) != 1:
                    ...
                    # Fix 2 & 3: weight SSS1, SSS2 and phase function by Csca*f(a)*da
                    SSS1 += ss1 * frequency * DK
                    SSS2 += ss2 * frequency * DK
            ...
            if len(a_) != 1 and THETA == self.th_min:
                ...
                self.ASYM = np.sum(ASYM_) / tot_freq
                self.QPR = np.sum(QPR_) / tot_freq
        ...
        f2.write("theta,I_perp,I_para,Polar,p_theta\n")

        if type(self.theta_in) == float or type(self.theta_in) == int:
            ITH = self.th_min + 1
            mie_theta(ITH, self.reff)
        else:
            for ITH in range(self.th_min + 1, self.th_max + 2, 1):
                # Print progress to match exact original behavior
