...
class mieshah2:
    ...
    def miecalc(self):
        ...
        def mie_theta(ITH,a):
            ...
            SS2=(S2R**2)+(S2I**2) 
            TSS=SS1+SS2
            Phase_func=2*TSS/(QSCA*(X**2))
            POLAR=(SS1-SS2)/TSS
    #        return X, QSCA, QEXT, QABS, ALBED, ASYM, QPR, QBAK, NN, THETA, POLAR, SS1, SS2,Pg
            self.X = X
            self.I_perp.append(SS1)
            self.I_para.append(SS2)
...
# Example usage
mies = mieshah2(ps=100, wl=6.283185307, m=(1.5,0), f='x**3', incr=0.1)
mies2 = mieshah2(ps=5, wl=0.6, m=(1.5,0), f='x**3', incr=0.1)
...
if mies.f is not None:
    print(mies.f.subs({mies.x:7}))             # Output: x**3
...
'''def X(**kwargs):
    if 'ps' in kwargs:
        print(f"Particle size= {kwargs['ps']}")
    if 'wl' in kwargs:
        print(f"Wavelength= {kwargs['wl']}")
    if 'm' in kwargs:
        print(f"Complex refractive index= {kwargs['m']}")
def X(**kwargs):
    if 'a' in kwargs and 'b' in kwargs:
        c = kwargs['a'] * kwargs['b']
        print(f"a*b = {c}")
    elif 'c' in kwargs and 'b' in kwargs:
        result = kwargs['c'] - kwargs['b']
        print(f"c-b = {result}")
    else:
