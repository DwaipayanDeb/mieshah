...
"""
Main Mie theory code for calculating scattering and absorption properties of spherical particles.

This module implements the Mie scattering theory to compute the scattering and absorption coefficients for spherical particles of a given size and refractive index. The code is based on the original Fortran implementation by Shah (1977) and has been converted to Python.

The algorithm iterates over angles from 0 to 180 degrees, calculating the scattering parameters for each angle. Results are written to output files for further analysis. The code supports up to 21,001 terms in the series expansion, which is sufficient for most practical applications.

Functions:
    Computes the scattering and absorption coefficients for spherical particles using Mie theory.
    Writes results to output files for further analysis.

Attributes:
    NMAX (int): Maximum number of terms in the series expansion (default: 21,001).
"""

P,Q,R,C,S,AR,AI,BR,BI=dim(21001),dim(21001),dim(21001),dim(21001),dim(21001),dim(21001),dim(21001),dim(21001),dim(21001)
f1=open("mie33.out","w")
f2=open("mie34.out","w")
f3=open("simulation.out","w")
SSS1=0.0
SSS2=0.0
NMAX=21001
a=100
