...
mu = np.cos(angles_rad)                # mu = cos(theta)
...
intensity = (I_parallel +I_perp)/2
    #intensity=iper+ipar
phase_function = intensity / np.sum(intensity * np.sin(np.radians(angles_deg)))
...
plt.yscale("log")
plt.plot(angles_deg, phase_function, label=r'PyMieScatt', color='red')
...
plt.title('Mie Scattering Intensities (PyMieScatt v/s Hapke Fig5.7a)')
