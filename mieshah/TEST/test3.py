...
class mieshah2:
    ...
    def __init__(self, **kwargs):
        ...
        if type(self.ps) == list:
            self.ps_min = min(self.ps)
            self.ps_max = max(self.ps)
        elif self.ps is not None and self.f is not None:
    ...
    #    a = self.ps
        ...
        def mie_theta(ITH,a_range):
            ...
            if type(a_range)==list and self.f is not None:
                a_=np.arange(a_range[0],a_range[1],DK)
            ...
            for a in a_:
                ...
                if len(a_)!=1:
                    ...
                    SSS1=SS1
                    SSS2=SS2
                Phase_func_list.append(np.pi*(SSS1+SSS2)/(QSCA*(X**2)))
            TSS=SSS1+SSS2
            Phase_func=np.mean(Phase_func_list)
            POLAR=(SSS1-SSS2)/TSS
        #        return X, QSCA, QEXT, QABS, ALBED, ASYM, QPR, QBAK, NN, THETA, POLAR, SS1, SS2,Pg
            self.X = X
            self.I_perp.append(SSS1)
            self.I_para.append(SSS2)
            self.scat_angle.append(THETA)
...
# Example usage
mies1 = mieshah2(ps=[0.001,0.62], wl=0.365, m=(1.403,0.024), f='x**-2', incr=0.005)
mies2 = mieshah2(ps=[0.62,6.2], wl=0.365, m=(1.403,0.024), f='x**-2.75', incr=0.01)
mies3 = mieshah2(ps=[6.2,20], wl=0.365, m=(1.403,0.024), f='x**-3.4', incr=0.1)
mies = mieshah2(ps=100, wl=6.283185307, m=(1.5,0.0))
...
plt.plot(mies3.scat_angle[::2], pol_av[::2], label='Polarization')
#plt.yscale('log')
plt.show()
